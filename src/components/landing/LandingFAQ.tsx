  ...
  return (
    <div className="border-b border-border">
      <button onClick={() => setOpen(!open)} className="w-full flex items-center justify-between py-5 text-left group cursor-pointer">
        <span className="font-body-en text-base font-semibold text-foreground group-hover:text-primary transition-colors">{q}</span>
        <ChevronDown size={18} className={`text-muted-foreground shrink-0 ml-4 transition-transform duration-200 ${open ? 'rotate-180' : ''}`} />
      </button>
      <div className={`overflow-hidden transition-all duration-300 ${open ? 'max-h-[200px] pb-5' : 'max-h-0'}`}>
        <p className="font-body-en text-[15px] text-muted-foreground leading-[1.7]">{a}</p>
      </div>
    </div>
  );
};
...
const LandingFAQ = () => {
  ...
  return (
    <section id="faq" ref={ref} className="py-[120px] px-6 bg-background">
      <div className={`max-w-[720px] mx-auto transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}`}>
        <h2 className="text-center font-heading-en font-[800] text-foreground" style={{ fontSize: 'clamp(28px, 4vw, 40px)' }}>Common questions</h2>
        <div className="mt-12">
