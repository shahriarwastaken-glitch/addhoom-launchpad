...
const LandingPricing = () => {
  ...
  return (
    <section id="pricing" ref={ref} className="py-16 sm:py-[120px] px-4 sm:px-6 bg-card">
      <div className="max-w-[1100px] mx-auto">
        <div className={`text-center mb-10 sm:mb-12 transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}`}>
          <span className="font-mono text-xs tracking-[0.1em] text-primary uppercase">Simple pricing</span>
          <h2 className="mt-4 font-heading-en font-[800] text-foreground" style={{ fontSize: 'clamp(28px, 4vw, 48px)' }}>Choose your plan.</h2>
          <p className="mt-3 font-body-en text-base sm:text-lg text-muted-foreground">Explore free. Subscribe when you're ready to start creating.</p>

          {/* Currency toggle */}
          <div className="mt-6 sm:mt-8 inline-flex items-center rounded-full p-1" style={{ background: '#F0EDE8' }}>
            <button onClick={() => setCurrency('intl')} className={`flex items-center gap-1.5 font-body-en text-sm font-medium rounded-full px-4 sm:px-5 py-2 transition-all ${currency === 'intl' ? 'bg-card shadow-[0_2px_8px_rgba(0,0,0,0.08)] text-foreground' : 'text-muted-foreground'}`}>
              International
            </button>
            <button onClick={() => setCurrency('bd')} className={`flex items-center gap-1.5 font-body-en text-sm font-medium rounded-full px-4 sm:px-5 py-2 transition-all ${currency === 'bd' ? 'bg-card shadow-[0_2px_8px_rgba(0,0,0,0.08)] text-foreground' : 'text-muted-foreground'}`}>
              Bangladesh
            </button>
          </div>
        </div>

        {/* Plan cards — stack on mobile, 2 cols on md, 4 cols on lg */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5 items-start">
          {plans.map((plan, i) => (
            <div
              key={plan.name}
              className={`relative rounded-[20px] p-6 sm:p-8 transition-all duration-700 hover:-translate-y-1 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'} ${plan.highlighted ? 'border-2 border-primary shadow-[0_8px_40px_rgba(255,81,0,0.12)] sm:lg:-translate-y-2 bg-card' : 'border-[1.5px] border-border bg-card'}`}
            ...
            >
              ...
              <div className="mt-4 flex items-baseline gap-1">
                <span className="font-heading-en font-[800] text-foreground" style={{ fontSize: 'clamp(36px, 5vw, 48px)' }}>{currency === 'intl' ? plan.priceIntl : plan.priceBD}</span>
          ...
          {/* Custom card */}
          <div className={`rounded-[20px] p-6 sm:p-8 transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}`} style={{ background: '#1C1B1A', transitionDelay: '300ms' }}>
            ...
            <p className="font-heading-en font-bold text-white mt-6" style={{ fontSize: 'clamp(24px, 3vw, 32px)', lineHeight: 1.2 }}>Need more?<br />Let's talk.</p>
            <a
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border">
                    <th className="text-left font-body-en font-semibold text-foreground px-3 sm:px-4 py-3">Action</th>
                    <th className="text-right font-body-en font-semibold text-foreground px-3 sm:px-4 py-3">Credits</th>
                  </tr>
                </thead>
                <tbody>
                  {creditCosts.map(c => (
                    <tr key={c.action} className="border-b border-border last:border-0">
                      <td className="font-body-en text-sm text-foreground px-3 sm:px-4 py-2.5">{c.action}</td>
                      <td className={`text-right font-mono text-sm px-3 sm:px-4 py-2.5 ${c.free ? 'text-brand-green font-semibold' : 'text-muted-foreground'}`}>
                        {c.credits}{c.free && ' ✓'}
                      </td>
                    </tr>
                  ))}
