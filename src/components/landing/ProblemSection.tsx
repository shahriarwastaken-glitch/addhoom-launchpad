...
const problems = [
  { emoji: '😤', stat: '৳৫,০০,০০০+', title: { bn: 'খারাপ বিজ্ঞাপনে নষ্ট হয়', en: 'Wasted on Bad Ads' }, desc: { bn: 'ব্যবসা শুরুর আগেই বাজেট শেষ।', en: 'Budget runs out before business starts.' } },
  { emoji: '⏰', stat: '৪০+ ঘণ্টা/মাস', title: { bn: 'ক্যাম্পেইন ম্যানেজ করতে যায়', en: 'Spent Managing Campaigns' }, desc: { bn: 'দোকান চালানোর সময় নেই।', en: 'No time to run your shop.' } },
  { emoji: '😵', stat: '৮৭%', title: { bn: 'উদ্যোক্তা মনে করেন মার্কেটিং কঠিন', en: 'Find marketing confusing' }, desc: { bn: 'টেকনিক্যাল জ্ঞান ছাড়া কঠিন।', en: 'Hard without technical knowledge.' } },
];
    ...
    <section ref={ref} className="py-24 px-4 bg-secondary">
      <div className="container mx-auto max-w-5xl">
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-12">
          {problems.map((p, i) => (
            <div key={i} className={`bg-card rounded-[20px] p-6 shadow-warm border-l-4 border-destructive transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}`}
              ...
              <div className="font-mono text-2xl font-bold text-foreground mb-1">{p.stat}</div>
        ...
        <div className="flex flex-wrap justify-center gap-3 mb-8">
          {['ফেসবুক অ্যাড ব্যর্থ', 'বাজেট শেষ', 'প্রতিযোগীরা এগিয়ে'].map(pill => (
            <span key={pill} className="text-sm border border-destructive/30 text-destructive rounded-full px-4 py-1.5 font-body-bn">{pill}</span>
          ))}
        </div>
