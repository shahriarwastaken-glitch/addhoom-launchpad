...
const formatNum = (n: number) => Math.floor(Math.abs(n)).toLocaleString('en-IN');
            ...
            <div className="space-y-8">
              <div>
                <div className="flex justify-between items-baseline mb-1">
                  ...
                  <span className="font-mono text-[22px] font-bold text-primary">{t(`৳${formatNum(adSpend)}`, `৳${formatNum(adSpend)}`)}</span>
                </div>
                ...
                <div className="relative h-1.5 rounded-full bg-border">
                  ...
                  <input type="range" min={5000} max={100000} step={1000} value={adSpend} onChange={e => setAdSpend(+e.target.value)} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
                ...
                <div className="flex justify-between mt-1.5 text-[11px] text-muted-foreground"><span>{t('৳৫,০০০', '৳5,000')}</span><span>{t('৳১,০০,০০০', '৳1,00,000')}</span></div>
              </div>
              <div>
                <div className="flex justify-between items-baseline mb-1">
                  <label className="text-sm font-semibold text-foreground font-bn">{t('বিজ্ঞাপনে সাপ্তাহিক ঘণ্টা', 'Weekly Hours on Ads')}</label>
                  <span className="font-mono text-[22px] font-bold text-primary">{weeklyHours} {t('ঘণ্টা', 'hrs')}</span>
                </div>
                ...
                <div className="relative h-1.5 rounded-full bg-border">
                  ...
                  <input type="range" min={2} max={40} step={1} value={weeklyHours} onChange={e => setWeeklyHours(+e.target.value)} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
            ...
            <div className="rounded-[20px] p-8" style={{ background: 'linear-gradient(145deg, hsl(var(--secondary)), hsl(var(--background)))' }}>
              ...
              <div className="hidden md:block">
                ...
                <div className="mt-3 space-y-2 text-sm font-bn">
                  {[
                    ...
                    <div key={i} className="flex items-center justify-between">
                      ...
                      <span className={item.positive ? 'text-brand-green font-mono' : 'text-muted-foreground font-mono'}>
                        {item.positive ? '+' : '-'}৳{formatNum(item.val)}
                      </span>
                    </div>
                  ))}
                  <div className="border-t border-border pt-2 flex justify-between font-bold text-primary">
                    ...
                    <span className="font-mono">৳{formatNum(results.monthlySavings)}</span>
                  </div>
                </div>
              </div>
              <div className="mt-6 border-l-[3px] border-brand-green bg-[#F0FFF8] dark:bg-brand-green/10 rounded-xl p-4">
                <p className="text-xs font-bn text-foreground leading-relaxed">
                  ...
                  {t(`আপনার ৳${formatNum(adSpend)} বাজেট থেকে আনুমানিক ৳${formatNum(results.roasGain)} বেশি রাজস্ব (২৫% ROAS উন্নতি)`, `From your ৳${formatNum(adSpend)} budget, est. ৳${formatNum(results.roasGain)} more revenue (25% ROAS boost)`)}
                </p>
              </div>
              <Link to="/auth" className="mt-6 block w-full text-center font-bn text-base font-bold text-primary-foreground bg-gradient-brand rounded-full py-4 shadow-orange-glow hover:scale-[1.02] transition-all duration-200">
                {t(`শুরু করুন — আজই ৳${formatNum(results.monthlySavings)} বাঁচান`, `Get Started — Save ৳${formatNum(results.monthlySavings)} today`)}
              </Link>
